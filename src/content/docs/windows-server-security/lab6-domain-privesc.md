...
DCSync 的核心是滥用 Active Directory 的**合法目录复制机制**。正常情况下，多台域控之间通过 DRS（Directory Replication Service，目录复制服务）协议同步数据。攻击者只要拥有足够的复制权限，就能假装自己也是一台域控，向真正的域控请求同步——从而拿到所有账号的密码哈希。
