...
本实验不是孤立地学习几个工具，而是围绕一条典型 Windows 本地账户攻击链展开：攻击者先想办法知道“有哪些账户”，再尝试把账户名变成可登录密码；一旦拿到一个管理员入口，就会进一步读取系统内存中的凭据，最后可能通过哈希重放或隐藏账户维持访问。因此，前置知识也按这条逻辑排列：先理解密码和认证，再理解凭据存放位置，最后理解后门账户为什么能被隐藏。

## 1. Windows认证协议体系

### 1.1 密码存储方式演进

在进入具体协议之前，先回答一个最基础的问题：Windows 到底保存的是什么？如果系统保存的是明文密码，攻击者只要读到数据库就能直接登录；现代 Windows 改为保存哈希值，降低了明文泄露风险。但哈希并不等于安全，尤其在 NTLM 场景下，哈希本身仍可能成为攻击材料。
...
上一节说明了 NTLM Hash 存在于 SAM 中，接下来要理解它为什么不仅是“存储结果”，还会参与认证过程。NTLM 认证不会把明文密码发给服务器，而是用哈希参与挑战/响应计算；这看似避免了明文传输，却也带来了一个关键问题：只要攻击者掌握哈希，就可能绕过明文密码完成认证。
...
理解 NTLM 的弱点后，再把它放到 Windows 的整体认证体系中比较会更清楚：工作组环境常见 NTLM，域环境默认使用 Kerberos。二者的区别决定了攻击面不同——NTLM 更关注哈希重放风险，Kerberos 更关注票据伪造和域控密钥保护。
...
## 2. SAM（Security Account Manager 安全账户管理器）数据库与LSASS（ Local Security Authority Subsystem Service本地安全机构子系统服务）进程

前面解决的是“Windows 如何验证身份”的问题，这一节转向“凭据存在哪里”。攻击者能否继续扩大战果，取决于他能读到哪些凭据材料：磁盘上的 SAM 决定了本地账户哈希是否会泄露，内存中的 LSASS 则决定了当前登录用户的密码、哈希和票据是否会被一次性带走。
...
SAM 更像是账户密码哈希的“静态仓库”，而 LSASS 是用户登录后的“动态凭据缓存”。这也是为什么阶段二拿到一个管理员密码后，阶段三不会止步于登录系统，而是继续读取 LSASS：目标从“使用一个账户”升级为“收集整台机器上已经登录过的凭据”。
...
前两节解释了攻击者如何获取凭据并扩大权限，但真实攻击往往不会在“拿到一次访问权限”后结束。为了在密码被修改、会话被清理后仍能回来，攻击者常会创建隐蔽账户作为持久化后门。因此，最后一个前置知识点从凭据提取自然过渡到后门排查：管理员不仅要看谁正在登录，还要看系统里是否多了“不容易被普通命令发现”的账户。
