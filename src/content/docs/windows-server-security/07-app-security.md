...
> 真实攻击中，后门程序是msfvenom生成的木马。教学环境中我们用一个无害的脚本模拟，重点理解服务后门的创建和检测流程。
...
> 💡 **说明**：Windows服务要求可执行文件实现SCM（服务控制管理器）接口，普通exe/bat直接作为服务启动会报错1053（超时）。这不影响本实验的核心目的——学习如何**创建、检测和删除**可疑服务。真实攻击中，攻击者使用的木马程序已实现SCM接口。
...
> 💡 **说明**：计划任务只是记录"何时执行什么程序"，即使目标程序不存在，任务也能成功创建。本实验重点是学习计划任务后门的创建、检测和清除流程。
...
```powershell
# ===== 在靶机（Windows Server）上执行 =====

# 启用WinRM服务
Enable-PSRemoting -Force

# 添加受信任主机（允许攻击机连接）— 实验环境设为*允许所有
...
> ⚠️ **预期结果**：`Enable-PSRemoting` 会输出多行配置信息并提示"WinRM 已更新"。`netstat` 应显示 `0.0.0.0:5985` 处于 LISTENING 状态。

**从Kali Linux攻击机连接（使用evil-winrm）**：

```bash
# Kali上安装evil-winrm（如未安装）
sudo gem install evil-winrm

# 连接靶机（需要靶机的管理员账号密码）
evil-winrm -i 192.168.100.20 -u Administrator -p 'P@ssw0rd'
...
> 💡 **为什么用evil-winrm？** Kali Linux没有原生PowerShell远程会话支持。evil-winrm是Kali内置的WinRM客户端工具，专为渗透测试设计，支持文件上传下载、内存加载.NET程序集等高级功能。
...
> 💡 **为什么需要PHP 5.6？** Upload-Labs的Pass-11/12（00截断漏洞）需要PHP < 5.3.4才能复现；Pass-13~16（图片马+文件包含）在所有PHP版本均可复现。教学中建议使用PHP 5.6作为折中——能复现大部分关卡，同时不会太过老旧。
...
> ⚠️ **前置条件**：本实验需要先完成实验8，确保phpStudy已启动且Upload-Labs可正常访问。同时需要安装Burp Suite Community Edition（免费版即可）。
