...
> 本实验的重点不是再新增一种后门，而是把前面学过的注册表、计划任务、服务、启动文件夹等自启动位置串起来，形成完整的排查思路。
...
> 🛡️ **最简单有效的防御**：不要只盯着某一种启动方式。实际排查时，应当按照“注册表 → 启动文件夹 → 计划任务 → 服务”的顺序做综合检查，这样更不容易遗漏。
...
| **项目二·用户管理** | 弱口令、高权限账户和不规范的账户授权，往往是后门植入和WebShell利用后的进一步扩大战果的基础 |
